...
public interface VueloRepository extends VueloRepositoryWithBagRelationships, JpaRepository<Vuelo, Long>, JpaSpecificationExecutor<Vuelo> {
    ...
    @Query(
        value = "select distinct vuelo from Vuelo vuelo left join fetch vuelo.aeropuerto left join fetch vuelo.destinoAeropuerto left join fetch vuelo.avion left join fetch vuelo.piloto",
        countQuery = "select count(distinct vuelo) from Vuelo vuelo"
    )
    Page<Vuelo> findAllWithToOneRelationships(Pageable pageable);

    @Query(
        "select distinct vuelo from Vuelo vuelo left join fetch vuelo.aeropuerto left join fetch vuelo.destinoAeropuerto left join fetch vuelo.avion left join fetch vuelo.piloto"
    )
    List<Vuelo> findAllWithToOneRelationships();

    @Query(
        "select vuelo from Vuelo vuelo left join fetch vuelo.aeropuerto left join fetch vuelo.destinoAeropuerto left join fetch vuelo.avion left join fetch vuelo.piloto where vuelo.id =:id"
    )
    Optional<Vuelo> findOneWithToOneRelationships(@Param("id") Long id);

    //métrica 2
    @Query("select v from Vuelo v where v.piloto.dni = ?1")
