...
public class VueloRepositoryWithBagRelationshipsImpl implements VueloRepositoryWithBagRelationships {
    ...
    public Optional<Vuelo> fetchBagRelationships(Optional<Vuelo> vuelo) {
        return vuelo.map(this::fetchTripulantes);
    }
    ...
    public List<Vuelo> fetchBagRelationships(List<Vuelo> vuelos) {
        return Optional.of(vuelos).map(this::fetchTripulantes).get();
    }

    Vuelo fetchTripulantes(Vuelo result) {
        return entityManager
            .createQuery("select vuelo from Vuelo vuelo left join fetch vuelo.tripulantes where vuelo is :vuelo", Vuelo.class)
            .setParameter("vuelo", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Vuelo> fetchTripulantes(List<Vuelo> vuelos) {
        return entityManager
            .createQuery("select distinct vuelo from Vuelo vuelo left join fetch vuelo.tripulantes where vuelo in :vuelos", Vuelo.class)
