...
export class VueloUpdateComponent implements OnInit {
  ...
  protected updateForm(vuelo: IVuelo): void {
    this.editForm.patchValue({
      ...
      destinoAeropuerto: vuelo.destinoAeropuerto,
      avion: vuelo.avion,
      piloto: vuelo.piloto,
      ...
      .pipe(
        map((aeropuertos: IAeropuerto[]) =>
          this.aeropuertoService.addAeropuertoToCollectionIfMissing(
            aeropuertos,
            this.editForm.get('aeropuerto')!.value,
            this.editForm.get('destinoAeropuerto')!.value
          )
        )
      )
    ...
    this.tripulanteService
      ...
      .pipe(
        map((tripulantes: ITripulante[]) =>
          this.tripulanteService.addTripulanteToCollectionIfMissing(tripulantes, ...(this.editForm.get('tripulantes')!.value ?? []))
        )
      )
  ...
  protected createFromForm(): IVuelo {
    return {
      ...
      destinoAeropuerto: this.editForm.get(['destinoAeropuerto'])!.value,
